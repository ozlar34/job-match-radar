...
Loads the calibration cases (cases.json) as a test set, runs the canonical
scoring prompt (scoring-prompt.md) through a judge, prints a
...
The guardrail (see `guardrail()`) mirrors the /job-digest-review HITL ritual:
independent of pass/fail, it flags an output for human review when the score
and its own rationale disagree (a cap-level score that names no disqualifier,
or a strong score whose rationale names a hard reject — the documented Company A-10
...
DEFAULT_PROMPT = os.path.join(HERE, "scoring-prompt.md")
...
# not applied — the Company A-10 failure mode).
HARD_REJECT_MARKERS = (
...
def guardrail(score, rationale, case, status):
    ...
    Independent of the pass/fail gate, this mirrors the /job-digest-review HITL
    ritual: surface outputs where the score and its stated reason disagree, or
